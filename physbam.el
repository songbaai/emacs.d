...
(defun physbam-grep-cpp-and-headers (directory_prefix querystr)
  ...
  (grep (concat "(find " path " -regex '.*/[0-9]+' -prune -o '(' '(' -name '*.h' -o -name '*.hpp' -o -name '*.cpp' ')' -print ')' 2>/dev/null | xargs grep -n -e \"" querystr "\") # "))))
...
(defun grep-cpp-and-headers (querystr)
  ...
  (grep (concat "(find . -regex '.*/[0-9]+' -prune -o '(' '(' -name '*.h' -o -name '*.hpp' -o -name '*.cpp' ')' -print ')' 2>/dev/null | xargs grep -n -e \"" querystr "\") # ")))
...
(setq compile-current-file-command "g++ -c -g -Wno-unused-local-typedefs -Wall -Werror -Winit-self -Woverloaded-virtual -Wstrict-aliasing=2 -std=gnu++17 -Wno-unknown-pragmas -Wno-strict-overflow -Wno-sign-compare -I$PHYSBAM/Public_Library -o /dev/null ")
...
(define-key global-map [menu-bar physbam edit-settings] '("Edit .emacs" . (lambda () (interactive) (find-file (concat (getenv "HOME") "/.emacs")))))
